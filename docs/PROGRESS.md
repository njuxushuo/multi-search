...
- Base、LoRA、Full SFT 的 v1 50k 局部续测均已完成并严格合并。完成时间依次为 2026-09-15 19:49、2026-09-16 00:39、2026-09-16 04:29；后台串行任务正常退出，GPU 0–3 已释放。
- v1 分别续测 11,958 / 23,596 / 22,738 道受影响题目，未受影响样本复用 v0。非空评分口径下，Base、LoRA、Full SFT 的最终 EM 为 18.496% / 24.370% / 28.792%，F1 为 22.551% / 29.319% / 34.226%。
- 相比 v0，三个模型分别新增 617 / 826 / 459 道 EM 正确题，真正的正确转错误均为 0；其中 522 / 816 / 458 道新增结果没有终止性格式或工具错误（允许 v1 定义的可恢复重复查询）。
- v1 平均实际 BM25 检索次数为 1.297 / 2.019 / 2.038，平均搜索尝试次数为 2.429 / 4.210 / 4.305。搜索尝试包含重复查询，中性提示拦截的重复查询没有再次调用 BM25。
- 已完成答案评分审计：旧 scorer 会把空预测与归一化后为空的 `!!!`、`---`、`The A` 等参考别名误判为正确。原始 JSONL/summary 保持只读；`docs/RESULTS.md` 已统一改用 prediction/gold 均非空的离线重算口径，并记录原始与修正数字。
- Full SFT v1 仍显著优于 LoRA，且二者实际检索成本接近。近期下一步为补充 Qwen3.5-4B-Instruct v1 基线、完成 Teacher 外部结果回传，并开展 BM25 上限与旧 Teacher 过滤损失审计；RL 继续暂缓。

## 2026-09-15 更新

- 当前第一版统一评测定义为 v0，完整协议已冻结在 `docs/EXPERIMENT_PROTOCOL.md`。
- Base、LoRA、Full SFT 的 v0 50k 评测均已完成并严格合并；当日原始 summary 中 Full SFT EM 27.892%、F1 33.134%，经 2026-09-16 非空评分审计后的正式值为 27.874% / 33.116%；平均成功检索 1.815 次、平均搜索尝试 2.266 次不受评分修正影响。27B Teacher 已在其他服务器评测，本机顺序脚本默认不再启动 Teacher（仅显式设置 `EVAL_RUN_TEACHER=1` 时启动）。
...
- v1 已按“重复查询不调用 BM25、返回中性 no-progress observation、最多 8 次搜索尝试后追加一次 answer-only generation”实现为独立 evaluator；v0 文件未修改，v1 输出固定隔离在 `outputs/eval/v1/`，代码核验已完成。
- v1 三模型顺序评测已于 2026-09-15 17:37 启动；本条记录为当时的历史运行状态，最终结果见上方 2026-09-16 更新和 `docs/RESULTS.md`。
