...
- v1 三模型顺序评测已于 2026-09-15 17:37 启动，顺序为 Base → LoRA → Full SFT，每个模型使用 GPU 0–3 四路数据并行。启动后约 10 分钟监控正常；Base 首批 512 条续接轨迹中新解出 29 条，其中 20 条同时满足严格格式，重建回退和 EM 损失均为 0。以上仅为运行中间值，最终数字以四 shard 严格合并后的 summary 为准。
