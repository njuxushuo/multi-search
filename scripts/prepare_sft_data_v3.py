...
from protocol_v3 import DEFAULT_CONFIG, compile_sft_example, file_sha256, load_protocol
...
def load_manifest(path: str, protocol_id: str) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    metadata = None
...
def load_selected_candidates(paths: list[str], selections: dict[str, dict[str, Any]], protocol_id: str) -> dict[str, dict[str, Any]]:
    ...
    for path_text in paths:
        with Path(path_text).open(encoding="utf-8") as handle:
            for line in handle:
                ...
                if row.get("protocol_id") != protocol_id or not row.get("strict_sft_eligible"):
...
def main() -> None:
    ...
    manifest_metadata, selections = load_manifest(args.selection_manifest, config["protocol_id"])
    candidates = load_selected_candidates(args.inputs, selections, config["protocol_id"])
